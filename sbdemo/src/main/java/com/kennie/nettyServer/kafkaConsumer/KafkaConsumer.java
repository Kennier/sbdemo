...
public class KafkaConsumer {
    ...
    // 批量消费监听
//    @KafkaListener(groupId = "consumerGroup1",topics = "${kafka.consume.topic.p2p}")
    @KafkaListener(topics = "${kafka.consume.topic.bind}")
    public void onBindMessage(ConsumerRecords<Object,String> consumerRecords) {
        for(TopicPartition topicPartition:consumerRecords.partitions()){
            for (ConsumerRecord<Object,String> consumerRecord:consumerRecords.records(topicPartition)){
                ...
                handleKafkaMsg.handleBind(consumerRecord.value());
            }
        }
    }
    ...
    // 批量消费监听
//    @KafkaListener(groupId = "consumerGroup1",topics = "${kafka.consume.topic.p2p}")
    @KafkaListener(topics = "${kafka.consume.topic.p2p}")
    public void onP2PMessage(ConsumerRecords<Object,String> consumerRecords) {
        for(TopicPartition topicPartition:consumerRecords.partitions()){
            for (ConsumerRecord<Object,String> consumerRecord:consumerRecords.records(topicPartition)){
                System.out.println("消费时间："+System.currentTimeMillis()+"  "+consumerRecord.value());
                handleKafkaMsg.handleP2P(consumerRecord.value());
            }
        }
    }

    // 批量消费监听
//    @KafkaListener(groupId = "consumerGroup1",topics = "${kafka.consume.topic.channel}")
    @KafkaListener(topics = "${kafka.consume.topic.channel}")
    public void onChannelMessage(ConsumerRecords<Object,String> consumerRecords) {
        for(TopicPartition topicPartition:consumerRecords.partitions()){
            for (ConsumerRecord<Object,String> consumerRecord:consumerRecords.records(topicPartition)){
                System.out.println("消费时间："+System.currentTimeMillis()+"  "+consumerRecord.value());
                handleKafkaMsg.handleChannelOrChannelEnter(consumerRecord.value());
            }
        }
    }

    // 批量消费监听
//    @KafkaListener(groupId = "consumerGroup1",topics = "${kafka.consume.topic.channelEnter}")
    @KafkaListener(topics = "${kafka.consume.topic.channelEnter}")
