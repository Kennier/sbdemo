...
public class SmartCarDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buffer,
        ...
        if (buffer.readableBytes() >= BASE_LENGTH) {
            ...
            // 防止，客户端传来的数据过大
            // 因为，太大的数据，是不合理的
            if (buffer.readableBytes() > 3500) {//3414是1000个文字
            ...
            while (true) {
                ...
                if (buffer.readInt() == ConstValue.HEAD_DATA) {//???readerIndex(5122) + length(4) exceeds writerIndex(5122): PooledUnsafeDirectByteBuf
                    break;
                }
